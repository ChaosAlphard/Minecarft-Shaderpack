...
const float LensFlareDelay = 2.000;
const float LensFlareNight = 0.800;
const float LensFlareSunRS = 0.825;
const float LensFlareDark  = 0.625;
    ...
    if (isEyeInWater < 0.9) {
        ...
        ///////////////////////////////////////////////////////////////////
			...
			if(timeMidnight < 0.5){
				...
				/*-----------Moon FRing-----------*/
				float Flare438 = FlarePointA(sP, lPos, 3.5, 3.5, flarescale, 3.05, 0.05, -2.0, sunmask);
					color.r += Flare438 * (1.05 - 0.7 * timeNoonNight) * FlareR;
					color.g += Flare438 *  0.4 						  * FlareG;
					color.b += Flare438 * (0.55 * timeNoonNight) 	  * FlareB;


				float Flare440 = FlarePoint(sP, lPos, 0.35, 10.0, flarescale, 2.0, 0.5, -2.0, sunmask);
					color.r += Flare440 * (1.2 - 0.8 * timeNoonNight) * FlareR;
					color.g += Flare440 *  0.5 						 * FlareG;
					color.b += Flare440 * (0.8 * timeNoonNight) 	  	 * FlareB;
				/*---------End Moon FRing---------*/
